...
OpenCode commands 与 agents 的官方仓库。提供项目无关的通用开发辅助命令和 sub-agent 模板。

## Commands 一览

| 命令 | 用途 |
|------|------|
| `sailing3d-commands` | 从本仓库同步 / 向本仓库推送 commands 和 agents |
...
| Agent | 模型 | 模式 | 用途 |
|-------|------|------|------|
| `reviewer-pro` | `deepseek/deepseek-v4-pro` | subagent (只读) | 独立代码审查与审计 |

使用方式: 在 Task tool 的 `subagent_type` 参数中指定 agent 名称。
...
2. 分别调用 GitHub API 获取两个目录列表:
   - `https://api.github.com/repos/sailing3d/opencode-commands/contents/.opencode/commands`
   - `https://api.github.com/repos/sailing3d/opencode-commands/contents/.opencode/agents`
3. 对返回的每个 `.md` 文件, 用 `download_url` 下载原始内容
4. 逐个保存到本地对应目录:
   - 命令文件 → `.opencode/commands/`
   - agent 文件 → `.opencode/agents/`
5. 在 `.opencode/` 下创建 `.bootstrap-source`, 内容为:
   ```
   url=https://github.com/sailing3d/opencode-commands
   branch=main
   ```
6. 完成后用表格列出所有命令文件及用途 (从 YAML `description` 提取), 以及所有 agent 文件
7. 提示: "Session restart 后新 commands 和 agents 生效"
...
将本地 commands 和 agents 推回本仓库，或用 fork 提交 PR。

### 新增 agent

1. 在 `.opencode/agents/` 下创建 `<agent-name>.md`
2. 在 [Agents 一览](#agents-一览) 中添加对应行
3. `sailing3d-commands push` 推送
