...
| 命令 | 用途 | Agent |
|------|------|-------|
| `/open-task <desc>` | 打开/开始任务. 可新建/继续doing/提升todo/重开done | build |
| `/update-task` | 将当前状况更新到任务文档. 无任务时提问 | build |
| `/close-task` | 完成任务归档到 done/ | build |
| `/todo-task <desc>` | 记录延后待办 | build |
| `/plan` | 分析需求, 讨论方案 | plan |
| `/build` | 按 Plan 决议执行, 内建 lint/typecheck 验证 | build |
| `/sync-docs` | 全局文档同步 (task docs + 知识库 + TODO + AGENTS) | build |
| `/warmup` | 围绕当前任务阅读文档填充上下文; 无任务时熟悉整个项目 | plan |
...
| `task-docs` | 任务文档生命周期管理 | open-task, close-task, update-task, todo-task, sync-docs |
